...
Write-Host "`n"
az connectedk8s connect --name $connectedClusterName --resource-group $env:resourceGroup --location $env:azureLocation --tags 'Company=Tailwind Manufacturing' --custom-locations-oid '51dfe1e8-70c6-4de5-a08e-e18aff23d815'
...
Start-Sleep -Seconds 20
az customlocation create --name 'taildata-cl' `
                         --resource-group $env:resourceGroup `
                         --namespace arc `
...
$customLocationId = $(az customlocation show --name "tailwind-data-cl" --resource-group $env:resourceGroup --query id -o tsv)
